import { mysqlTable, mysqlSchema, AnyMySqlColumn, index, foreignKey, primaryKey, int, time, varchar, text, unique, datetime, date, mysqlEnum } from "drizzle-orm/mysql-core"
...
export const log = mysqlTable("Log", {
	...
	detalhe: text(),
	dataHora: datetime("data_hora", { mode: 'string' }).default(sql`(CURRENT_TIMESTAMP)`),
},
	(table) => [
		index("id_usuario").on(table.idUsuario),
...
export const usuario = mysqlTable("Usuario", {
	...
	dataCriacao: datetime("data_criacao", { mode: 'string' }).default(sql`(CURRENT_TIMESTAMP)`),
...
export const sessao = mysqlTable("Sessao", {
	...
	criadoEm: datetime("criado_em", { mode: 'string' }).default(sql`(CURRENT_TIMESTAMP)`),
